...
public record CreateTestResultCommand(CreateTestResultViewModel Model) : IRequest<Result<TestResultViewModel>>, IRequireAuthorization
{
    public Task<bool> CanAccess(IUser user)
    {
        return Task.FromResult(user.Role is Role.Organizer || user.Role is Role.Judge);
    }
}
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var entry = await _unitOfWork
                ...
                .InsertAsync(testResult, cancellationToken);
            ...
            await _eventPublisher.PublishAsync(new TestResultCreatedEvent(viewModel), cancellationToken);
            return Result<TestResultViewModel>.Ok(viewModel).SetMessage("Баллы сохранены");
        }
        catch (Exception e)
        {
            await transaction.RollbackAsync(cancellationToken);
            return Result<TestResultViewModel>.Fail(e.Message);
        }
    }
